...
from tensorflow.keras.layers import Dropout, Flatten, Cropping2D, Lambda
...
class KerasPilot:
    ...
    def train(self, train_gen, val_gen, saved_model_path, epochs=100, steps=100, train_split=0.8, verbose=1,
              min_delta=.0005, patience=5, use_early_stop=True, redis_connect=None, task_id=None):
        """
        ...
        log_save = keras.callbacks.LambdaCallback(on_epoch_begin=None,
                                                  on_epoch_end=lambda epoch, logs: redis_connect.hset(task_id, mapping={
                                                      "epoch": epoch+1, "loss": logs['loss']}),
                                                  )

        callbacks_list = [save_best, log_save]

        if use_early_stop:
            callbacks_list.append(early_stop)

        history: keras.callbacks.History = self.model.fit_generator(
            train_gen,
